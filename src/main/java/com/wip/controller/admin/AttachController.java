    ...
    public APIResponse filesUploadToCloud(
        ...
        try {
            ...
            for (MultipartFile file :files) {
                ...
                String rootFilePath = System.getProperty("user.dir") + "\\files\\" + day;  // 获取上传的路径
                File saveFile = new File(rootFilePath);
                if (!saveFile.exists()) {
                    saveFile.mkdirs();
                }
                File f = new File(rootFilePath + "/" + fKey);
                ...
                attAchDomain.setFtype(TaleUtils.isImage(inputStream) ? Types.IMAGE.getType() : Types.FILE.getType());
                attAchDomain.setFname(rootFilePath + "/" + fKey);
